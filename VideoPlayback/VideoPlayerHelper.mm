...
//    (void)AudioSessionSetActive(true);
    ...
    if (NOT_READY != mediaState && ERROR != mediaState) {
        ...
        if (NSNotFound == [filename rangeOfString:@"://"].location) {
            ...
            NSLog(@"Media  loaded.   current media first.");
...
- (BOOL)play:(BOOL)fullscreen fromPosition:(float)seekPosition
{
    ...
    if ((PLAYING != mediaState || playerType != requestedPlayerType) && NOT_READY > mediaState) {
        if (PLAYER_TYPE_NATIVE == requestedPlayerType) {
            ...
            [app rootViewControllerPresentViewController:movieViewController inContext:NO];
...
-(BOOL)playUrlVideo:(BOOL)fullscreen view:(id)assignView fromPosition:(float)seekPosition{
    BOOL ret = NO;
...
//        if (PLAYER_TYPE_NATIVE == requestedPlayerType) {
            ...
            UIView *view=(UIView *)assignView;
            AVURLAsset *avasset = [[AVURLAsset alloc] initWithURL:mediaURL options:nil];
             ...
             dispatch_async(dispatch_get_main_queue(),
                            ^{
                                ...
                                if (status == AVKeyValueStatusLoaded) {
                                    ...
                                    CGSize size = view.bounds.size; //self.bounds.size;
